defmodule BetterForms.Invoices.InvoiceTest do
  ...
  describe "create_with_dollars_changeset/2" do
    setup do
      invoice_attrs =
        build(:invoice)
        |> Map.from_struct()
    ...
    test "requires an amount_in_dollars", %{invoice_attrs: invoice_attrs} do
      changeset =
        invoice_attrs
        |> Map.put(:amount_in_dollars, nil)
        |> Invoice.create_with_dollars_changeset()

      assert "can't be blank" in errors_on(changeset).amount_in_dollars
    end

    test "rejects negative amount_in_dollars", %{invoice_attrs: attrs} do
      changeset =
        attrs
        |> Map.put(:amount_in_dollars, -7)
        |> Invoice.create_with_dollars_changeset()

      assert "must be greater than 0" in errors_on(changeset).amount_in_dollars
    end

    test "rejects zero amount_in_dollars", %{invoice_attrs: attrs} do
      changeset =
        attrs
        |> Map.put(:amount_in_dollars, 0)
        |> Invoice.create_with_dollars_changeset()

      assert "must be greater than 0" in errors_on(changeset).amount_in_dollars
    end

    test "allows a valid amount_in_dollars", %{invoice_attrs: attrs} do
      changeset =
        attrs
        |> Map.put(:amount_in_dollars, 12_3.0)
        |> Invoice.create_with_dollars_changeset()

      assert changeset.valid?
      assert changeset.changes.amount_in_cents == 12_300
    end

    test "allows a valid decimal amount_in_dollars", %{invoice_attrs: attrs} do
      changeset =
        attrs
        |> Map.put(:amount_in_dollars, 12_3.45)
        |> Invoice.create_with_dollars_changeset()
